...
def candidate_request_wrapper(
    ...
    1. Takes the detailed task format and the endpoint for candidate generation.
    2. Translates it into the simpler format expected by the reused `send_sglang_request`.
    3. Calls the reused function to interact with a specific SGLang server endpoint.
    4. Reformats the output into the final, detailed record for our dataset.
    """
    task, endpoint = task_and_endpoint
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate candidate responses from multiple data sources via multiple running SGLang servers.",
    ...
    parser.add_argument("--host", type=str, default="localhost", help="Host of the SGLang servers.")
    parser.add_argument("--base_port", type=int, default=30000, help="Base port of the SGLang servers.")
    parser.add_argument("--num_servers", type=int, default=1, help="Number of SGLang server instances to distribute requests to.")
    ...
    endpoints = [f"http://{args.host}:{args.base_port + i}/generate" for i in range(args.num_servers)]
    print(f"Distributing tasks across {len(endpoints)} endpoints: {endpoints}")

    tasks_and_endpoints = list(zip(all_tasks_to_run, cycle(endpoints)))
    session = requests.Session()
    
    with open(args.output_path, 'w', encoding='utf-8') as f_out:
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            future_to_task = {
                executor.submit(candidate_request_wrapper, task_and_endpoint, session, tokenizer, args): task_and_endpoint
                for task_and_endpoint in tasks_and_endpoints
            }
